...
3. Group pending work by module or component and order it by real dependencies (what blocks what), not by arrival order or unsupported perceived priority.
4. For each in-progress or upcoming activity, assign an agent (or mark it "unassigned" if none is available) and define explicit, verifiable acceptance criteria — not generic ones like "it works," but checkable conditions: specific tests green, CI green, editorial contract or other interface contract unchanged without authorization, etc.
5. Generate the complete, self-contained prompt each agent with an assigned activity should receive: enough context to work without depending on this conversation, exact scope, explicit boundaries on what NOT to touch, and the acceptance criteria from step 4.
...
7. When you receive an agent's output, verify it against that activity's acceptance criteria citing concrete evidence (diff, test result, CI log, PR number) — don't accept a "done" report without that evidence.
...
9. Re-issue the complete, updated work plan after every review cycle, not just a change summary — whoever reads it should be able to resume the program without additional context.
...
- agent assignment and acceptance criteria per activity,
- complete, self-contained prompt for each agent with an upcoming activity,
- active risks and proposed mitigation,
- a review verdict for every agent output received this cycle, with evidence cited.
```
