...
### Added
- **Programa Fundador — mecanismo de precio congelado resuelto e infraestructura del contador (decisión 2026-08-09)**: se verificó que el "precio congelado de por vida" no requiere código de cobro — en Paddle Billing los cambios de precio solo afectan suscripciones nuevas (grandfathering por defecto); al terminar el piloto se crea un price nuevo y se actualiza `PADDLE_PRICE_ID`, sin migrar jamás a la cohorte fundadora. Decisión de producto "a": los suscriptores 51+ previos a la subida también conservan su precio (default de Paddle, más generoso que lo prometido, cero churn inducido). Lo que sí faltaba y se implementó: `subscriptions.created_at` (orden de llegada de la cohorte, fijado al primer insert) y el RPC público de solo-agregado `founding_spots_left()` (`supabase/migrations/20260809_founding_members.sql`, también en el `subscriptions.sql` base para instalaciones nuevas), más el banner Fundador en `/precios` (`build.py`) — oculto por defecto y fail-closed: solo aparece con lugares reales devueltos por el RPC y checkout de Paddle configurado, nunca con números inventados. Un lugar se consume al crearse la suscripción y no se libera al cancelar. Actualizados `docs/marketing/early-adopters-program.md` (mecanismo y checklist), `docs/paddle-integration.md` (paso de migración) y `docs/STRATEGY.md` (decisión registrada + ítem 13).

### Changed
- **Los documentos vivos dejan de hardcodear "$1 USD/mes" como precio vigente**: al activar el banner del Programa Fundador en producción se detectó que la prosa de `docs/STRATEGY.md`, `docs/requirements/BusinessRules.md` (MR-02/MR-09), `docs/paddle-integration.md`, `README.md`, `docs/gumroad-listing.md`, el `LEEME` del pack (`build_gumroad_pack.py`) y los materiales de campaña (`early-adopters-program.md`, `outreach-ctos.md`) seguía citando $1 USD/mes, mientras el sitio en producción cobra y publica $9 USD/mes (ancla $19) desde `PADDLE_PRICE_AMOUNT_USD` — el mismo defecto de precio-en-prosa que ya se había corregido dentro de `build.py`. Ahora la prosa apunta a `/precios` como fuente de verdad (citando $9 solo con fecha "al 2026-08-09" donde el copy necesita un número, con instrucción explícita de verificar contra la página antes de publicar), y las menciones históricas de $1 quedan marcadas como históricas ("entonces"). Ningún cambio en el sitio generado: el monto ya derivaba de la configuración.
- **Precisión de la escala de autonomía en materiales de marketing**: `docs/marketing/dev-to-article.md` y el post 3.2 de `docs/marketing/linkedin-posts.md` parafraseaban A2 como "ejecutar cambios acotados de bajo riesgo" y A3 como "ejecución con supervisión mínima", cuando la escala canónica de `00-framework.md` define A2 = Ejecutar controlado (editar/validar solo en workspace o rama aislada) y A3 = Publicar (commit, push, PR o despliegue, con aprobación explícita o política preautorizada). Corregidos ambos para citar la escala real antes de su publicación — el público objetivo es técnico y puede verificar contra el catálogo.
